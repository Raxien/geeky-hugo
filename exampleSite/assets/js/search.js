...
function setupRealTimeSearch() {
  ...
  loadFuse()
    ...
    .catch(error => {
      ...
      searchResults.innerHTML = '<div class="search-result-item">{{ i18n "search_error" }}</div>';
    });
  ...
  searchInput.addEventListener('input', function() {
    ...
    searchTimeout = setTimeout(() => {
      if (!fuse) {
        searchResults.innerHTML = '<div class="search-result-item">{{ i18n "search_loading_in_progress" }}</div>';
      ...
      if (results.length > 0) {
        ...
        searchResults.innerHTML = `
          <div class="search-results-list">
            ${limitedResults.map(result => {
              ...
              const title = item.title || '{{ i18n "search_untitled" }}';
        ...
        searchResults.innerHTML = '<div class="search-result-item">{{ i18n "no_results_found" }}</div>';
        searchResults.style.display = 'block';
      }
    }, 300);
  });
